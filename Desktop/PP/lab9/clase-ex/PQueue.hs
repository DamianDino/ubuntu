...
class (Ord a) => PQueue pq a where
    ...
    toList a
        | (top a) == Nothing = []
        | otherwise = (fromJust (top a)) : (toList (pop a))


    size :: pq a -> Int
    size a
        | isEmpty a = 0
        | otherwise = 1 + size (pop a)  -- Pentru BONUS 6
...
instance (Ord a) => PQueue ListPQ a where
    ...
    insert (pri, x) q@(LPQ queue)
        | isEmpty q = LPQ [(pri, x)]
        | pri >= (fst (fromJust (top q))) = LPQ ((pri,x ) : queue)
        | otherwise = LPQ ((fromJust (top q)) : (toList (insert (pri,x) (pop q))))
...
instance (Ord a) => PQueue LeftistPQ a where
    ...
    isEmpty pq = undefined
    
    insert elem pq = undefined
