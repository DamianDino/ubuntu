...
myConcat([], L, L).
myConcat([X|Rest], L, [X|Rez]) :- myConcat(Rest, L, Rez).
...
%myReverse(_,_):- fail.
myReverse([], []).
myReverse([X|Rest], RevList) :- myReverse(Rest, ReversedRest), myConcat(ReversedRest, [X], RevList).
...
myReverseAcc([X|Rest], Acc, RevList) :- myReverseAcc(Rest, [X|Acc], RevList).
...
extract([], _, _, []).
extract([Head | Rest], 0, End, [Head | Range]) :- End >= 0, End1 is End - 1, extract(Rest, 0, End1, Range).
extract([_ | Rest], Start, End, Range) :- Start1 is Start-1, End1 is End-1, extract(Rest, Start1, End1, Range).
...
factorial_helper(A,B,Rez,X) :- B >= Rez, X is A.
factorial_helper(A,B,Rez,X) :- B < Rez, A1 is A + 1, B1 = B * A1, factorial_helper(A1,B1,Rez,X).
...
%sublista(_,_):- fail.
sublista([],[]).
sublista([_|Rest], L) :- sublista(Rest, L).
sublista([X|Rest1], [X|Rest2]) :- sublista(Rest1, Rest2).
...
%isLeaf(_):- fail.
%isLeaf(Node) :- arc(_,Node), \+ arc(Node, _) ^ \+ arc(_,Node), \+ arc(Node, _).
...
descendantOf(X,Z) :- arc(Y,X), descendantOf(Y,Z).
...
descendants(A, N) :- arc(A, B), \+ (arc(A, D), D \= B), descendants(B, N1), N is N1+1.
descendants(A, N) :- arc(A, B), arc(A, C), B @< C, descendants(B, N1), descendants(C, N2), N is N1+N2+2.
...
%sameTree(_,_):- fail.
sameTree(A, A).
sameTree(A, B) :- arc(A, B); descendantOf(A, B); descendantOf(A, G), descendantOf(B, G).
...
next(Nod1, Nod2) :- arc(Nod1, Nod2), Nod1 \= Nod2.
prev(Nod1, Nod2) :- arc(Nod2, Nod1), Nod1 \= Nod2.

drumDirect(S,D,[S|Path]) :- next(S, S1), drum(S1, D, Path).
drumInvers(S,D,Path) :- prev(S, S2), drum(S2, D, Path).

drum(A,A,[A]).

drum(S, D, Path) :- drumDirect(S,D,Path); drumInvers(S,D,Path).
