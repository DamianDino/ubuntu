...
arithmetic init a = iterate(\x -> x+a) init
geometric :: Num a => a -> a -> [a]
geometric init a = iterate(\x -> x*a) init
{-
  3. (2p)
...
randoms :: Int -> [Word8]
randoms seed = tail $ map (fromIntegral . fst) $ iterate (next . snd) (1, mkStdGen seed)
{-
    4. (1p)
...
wordToAlpha number = chr ((mod (fromIntegral (number::Word8)::Int) 26) + 97)
{-
    5. (1p)
...
tableToFunc lista caracter = snd ((!!) lista ((ord caracter) - 97))
