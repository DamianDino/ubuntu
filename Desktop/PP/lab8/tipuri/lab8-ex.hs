...
normalizeV v@(V vx vy vz) = V (vx * raport) (vy * raport) (vz * raport)
	where raport = 1 / (lengthV v)
...
computeDot :: [Vector] -> [Double]
computeDot [] = []
computeDot (v:vs) = [dotV v a | a <- vs] ++ computeDot vs


orthogonalV :: [Vector] -> Bool
orthogonalV = null . filter (/= 0.0) . computeDot
